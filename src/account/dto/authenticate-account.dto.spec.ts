  ...
  describe('email', () => {
    ...
    it('should fail validation if email is empty', async () => {
      ...
      expect(
        errors.some(
          (error) =>
            error.property === 'email' && error.constraints?.isNotEmpty,
        ),
      ).toBe(true);
    });
    ...
    it('should fail validation if email does not match the regex', async () => {
      ...
      expect(
        errors.some(
          (error) => error.property === 'email' && error.constraints?.matches,
        ),
      ).toBe(true);
    });
    ...
    it('should fail validation if email exceeds the maximum length', async () => {
      ...
      expect(
        errors.some(
          (error) => error.property === 'email' && error.constraints?.maxLength,
        ),
      ).toBe(true);
    });
  });
  ...
  describe('password', () => {
    ...
    it('should fail validation if password is not a string', async () => {
      ...
      expect(
        errors.some(
          (error) =>
            error.property === 'password' && error.constraints?.isString,
        ),
      ).toBe(true);
    });
    ...
    it('should fail validation if password is too long', async () => {
      ...
      expect(
        errors.some(
          (error) =>
            error.property === 'password' && error.constraints?.maxLength,
        ),
      ).toBe(true);
    });
    ...
    it('should fail validation if password does not match the regex', async () => {
      ...
      expect(
        errors.some(
          (error) =>
            error.property === 'password' && error.constraints?.matches,
        ),
      ).toBe(true);
    });
  });
